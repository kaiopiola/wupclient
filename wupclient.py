...
class wupclient:
    ...
    def __init__(self, ip='192.168.0.128', port=1337):
        self.s=socket.socket()
        
        self.s.connect((ip, port))
        self.fsa_handle = None
        self.cwd = "/vol"
        self.pcdir = os.getcwd()
        self.size_pc = [] 
        self.size_wiiu = []
    ...
    def cd(self, path):
        ...
        if ret == 0:
            self.cwd = wpath or self.cwd
            self.FSA_CloseDir(fsa_handle, dir_handle)
            return 0
        else:
            wpath = os.path.normpath(path).replace("\\\\", "/").replace("\\", "/").split("/")
            if len(wpath) == 2 and wpath[1] == "vol":
                self.cwd = "/vol"
                print "Using simulated /vol"
                return 0
    ...
    def ls(self, path = None, return_data = False):
        fsa_handle = self.get_fsa_handle()
        tpath = path if path != None else self.cwd
        ...
        if len(wpath) == 2 and wpath[1] == "vol":
            ...
            if return_data:
                return ["system_slc", "storage_mlc01", "storage_odd_tickets", "storage_sdcard", "storage_usb", "storage_odd_update"]
            return
        
        ret, dir_handle = self.FSA_OpenDir(fsa_handle, tpath)
        if ret != 0x0:
    ...
    def rmdir_recursive(self, path):
        ...
        entries = self.ls(path, True)
        for e in entries:
            item_path = path + "/" + e["name"]
            if e["is_file"]:
                self.rm(item_path)
            else:
                self.rmdir_recursive(item_path)
        # Agora a pasta esta vazia, pode remover
        ret, dir_handle = self.FSA_OpenDir(fsa_handle, path)
        if ret == 0:
            self.FSA_CloseDir(fsa_handle, dir_handle)
        ret = self.FSA_Remove(fsa_handle, path)
        print("rmdir_recursive : " + str(path) + " => " + hex(ret))
